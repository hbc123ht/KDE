from sklearn.neighbors import KernelDensity
import numpy as np
import matplotlib.pyplot as plt
import seaborn as sns; sns.set()
from sklearn.model_selection import GridSearchCV
from sklearn.model_selection import LeaveOneOut
from sklearn.datasets import load_digits
from sklearn.decomposition import PCA
from mnist import MNIST
import math
def loaddata():
    mnist = MNIST('/Users/cong/Downloads/MNIST')#copy the path consisting the data file here
    x_train, y_train = mnist.load_training()  # 60000 samples
    x_test, y_test = mnist.load_testing()
    x_train = np.asarray(x_train).astype(np.float32)
    y_train = np.asarray(y_train).astype(np.int32)
    x_test = np.asarray(x_test).astype(np.float32)
    y_test = np.asarray(y_test).astype(np.int32)
    data = [[0] * 784] * 1000
    data = np.array(data)
    datal = [[0] * 794] * 1000
    datal = np.array(datal)
    for i in range(1000):
        for j in range(784):
            data[i,j] = x_train[i,j]
            datal[i,j] = x_train[i,j]
            datal[i][y_train[i] + 784] = 1000000
    return (data,datal,y_train,x_test)
def mer(data,x):
    datal = [0] * 794
    for i in range(784):
        datal[i] = data[i]
    datal[x + 784] = 1000000
    return datal
class KDE(object):
    def __init__(self,data,K):
        self.data = np.array(data)
        self.K = K
    def dot(self,x,y):
        s = 0
        for i in range(x.shape[0]):
            tmp = x[i] - y[i]
            s += (tmp ** 2)
        return np.sqrt(s)

    def dis(self,x):
        num = 0.0
        for i in range(self.data.shape[0]):
            num += 1 / (self.data.shape[0] * self.K * np.sqrt(2 * np.pi)) * np.exp( -(self.dot(self.data[i],x)) ** 2 / (2 * (self.K ** 2) * 100))
        return num

data,datal,y_train,x_test = loaddata()
KDE1 = KDE(data,37.64935806792467)
KDE2 = KDE(datal,35.2970730273065)
"""
for i in range(3):
    #if (KDE2.dis(mer(data[i],5)) / KDE1.dis(data[i]) > KDE2.dis(mer(data[i],1)) / KDE1.dis(data[i])):
     #   print("success")
    print(KDE2.dis(mer(data[i],5)) / KDE1.dis(data[i]))
    """
result = [0] * 784
num = 0 #chosse the number to generate here
c = 0
while (1 == 1):
    c += 1
    tmp = KDE2.dis(mer(x_test[c], num)) / KDE1.dis(x_test[c])
    print(tmp)
    if (tmp >= 0.3):
        result = x_test[c]
        break

plt.figure()
plt.imshow(result.reshape(28,28))
plt.show()
# in order to reduce the runtime, i already predicted the bandwidth with the code following:
"""
params = {'bandwidth': np.logspace(-1, 2, 200)}
grid = GridSearchCV(KernelDensity(), params)
grid.fit(data)
K = grid.best_estimator_.bandwidth
"""
